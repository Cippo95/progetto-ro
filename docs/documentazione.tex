...
Ho scritto il codice per fornire una soluzione ammissibile al problema in Java.
...
\subsection{Ottimalità della soluzione (?)}
La soluzione fornita dal mio programma è solo una soluzione ammissibile, si può vedere facilmente che non sarebbe ottima in caso un job \textit{ready} abbia \textit{ratio} maggiore di un altro \textit{ready} e da lui dipendono altri job a \textit{ratio} molto basso (se è così il valore $>>$ durata e quindi farli prima minimizza la somma pesata) che quindi non partono quando invece sarebbe stato conveniente far partire lui anche se peggiore e aggiungere gli altri job\ldots se non avessero altre dipendenze! il che fa capire che il problema è difficile.
Cercando online infatti il problema è stato dimostrato \textit{NP-Hard}\cite{np-hard}, dovrei quindi immaginare un procedimento un po' più complesso della mia semplice greedy costruttiva per avere una soluzione migliore.
Probabilmente la strada migliore sarebbe quella di affidarsi a qualche tool di ricerca operativa, sto dando un'occhiata agli ``OR-Tools'' della Google, però probabilmente non farò in tempo a riadattare il mio progetto con questi tool.
